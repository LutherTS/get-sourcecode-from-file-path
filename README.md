 ...
 * @param {import('eslint').Linter} [options.linter] The `Linter` instance used to retrieve the `SourceCode` object, defaulting to a `new Linter()` per operation ensuring each call of `getSourceCodeFromFilePath` is based on its own linter (just in case somehow some linters were to run concurrently).
